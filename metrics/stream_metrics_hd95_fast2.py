...
def hd95(pred, label):
    ...
    for i in range(pred.shape[0]):
        ...
        # Skip if either mask has no foreground
        if np.sum(pred_bin) == 0 or np.sum(label_bin) == 0:
            continue

        try:
            distance = binary.hd95(pred_bin, label_bin)
            dist += distance
            valid_count += 1
        except Exception as e:
            print(f"HD95 error at sample {i}: {e}")
            continue

    # Avoid division by zero
    return dist if valid_count == 0 else dist / valid_count


"""---------------代码没错,运行时间很长"""
def hd95Dan(pred, label):
    dist = 0
    for i in range(pred.shape[0]):
        ...
        pred_tp[bool_pred_j == True] = 1
        tg_tp[bool_tg_j == True] = 1
        distance = metric.hd95(pred_tp, tg_tp)
        dist += (distance *  0.95)
