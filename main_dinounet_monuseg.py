"""
MoNuSeg segmentation training pipeline using the DiNOUNet backbone
(DINOv3 ViT-B/16 + UNet decoder).

Mirrors the structure of main_dino_glas.py:
- Single integrated main: train + per-epoch validation + final test on
  the held-out MoNuSeg test folder.
...
- All outputs (logs, checkpoints, results, metrics CSV) are written under
  ./runs/monuseg_dino_v1/.
- Adds an early-stopping mechanism on val_dice (patience configurable).
"""
...
def make_print_to_file(path=RUN_DIR):
    ...
    fileName = datetime.datetime.now().strftime('DAY%Y_%m_%d_') + 'MoNuSeg_DiNOUNet.log'
...
def get_argparser():
    ...
    parser.add_argument("--train_dataset", type=str,
                        default='/data3/nkozah/my_project/Data/MoNuSeg/train')
    parser.add_argument("--test_dataset", type=str,
                        default='/data3/nkozah/my_project/Data/MoNuSeg/test')
    ...
    # Training hyper-parameters (mirrors the DeepLab / TransUNet main)
    ...
    parser.add_argument("--early_stopping", action='store_true', default=True,
                        help="Enable early stopping on val_dice plateau.")
    parser.add_argument("--patience", type=int, default=50,
                        help="Number of epochs without val_dice improvement "
                             "before stopping.")
    parser.add_argument("--min_delta", type=float, default=1e-4,
                        help="Minimum val_dice improvement to reset the "
                             "early-stopping counter.")
    parser.add_argument("--min_epochs", type=int, default=20,
                        help="Disable early stopping for the first N epochs "
                             "(warm-up grace period).")
    return parser


# -----------------------------------------------------------------------------
# LR schedule (identical to the DeepLab / TransUNet main)
# -----------------------------------------------------------------------------
...
def main():
    ...
    # ----- Model -----
    model_name = os.path.join(RUN_DIR, 'checkpoints', 'MoNuSeg_DiNOUNet_best.pth')
    ...
    txt_results = os.path.join(RUN_DIR, f'{tm}_MoNuSeg_DiNOUNet_results.txt')
    ...
    for epoch in range(opts.START_EPOCH, opts.NB_EPOCH):
        ...
        # ----- Best-model bookkeeping + early-stopping counter -----
        improved = (val_dice - best_dice) > opts.min_delta
        if improved:
            best_dice = val_dice
            best_iou = val_iou
            best_epoch = epoch
        ...
        line = (f"Epoch {epoch} | lr={lr:.2e} | "
                ...
                f"no_improve={epochs_no_improve}/{opts.patience}")
        print(line)
        ...
        if (opts.early_stopping
                and epoch >= opts.min_epochs
                and epochs_no_improve >= opts.patience):
            print(f"[EarlyStop] No val_dice improvement for {epochs_no_improve} epochs "
                  f"(>= patience={opts.patience}). Stopping at epoch {epoch}. "
                  f"Best val_dice={best_dice:.4f} @ epoch {best_epoch}.")
            with open(txt_results, 'a') as f:
                f.write(f"[EarlyStop] epoch={epoch} best_val_dice={best_dice:.4f} "
    ...
    print("\n=========== Final test on MoNuSeg test set ===========")
    ...
    test_line = (f"TEST | early_stopped={stopped_early} | "
                 f"best_epoch={best_epoch} | best_val_dice={best_dice:.4f} | "
