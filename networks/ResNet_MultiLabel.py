...
model = UNetWithResnet50Encoder(n_classes=9).cuda()
...
class UNet_R50_3Heads_1Decoder(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.seg_head = nn.Conv2d(64, n_classes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, train=False, superpixel_map=None):
        ...
        # # else:
        # #     return x
        seg_out = torch.softmax(self.seg_head(x), dim=1)
        ...
        if train==False:
            ...
            return seg_logits, recontrust_out, SurPLocalCls_batch
...
model = UNet_R50_3Heads_1Decoder(9).cuda()
inp = torch.rand((2, 3, 224, 224)).cuda()
out = model(inp)
print(out.shape)